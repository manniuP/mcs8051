...
// uart_echo 便携构建：纯 Zig UART1 接收中断回环。只用上一级的共享 toolchain/。
// 流程：[1] 自带 zig 的 MCS 后端 build-obj -> uart_echo.asm
//       [2..4] mcstools.exe（fix/opt/ir/overlay 后处理，无需 Python）
//       [5] sdas{251,8051} 汇编 + sdcc 链接（crt0.asm + uart_echo.rel）-> uart_echo.ihx
//
...
pub fn build(b: *std.Build) void {
    ...
    // [2/5][3/5][4/5] 构建层后处理（修标签重名 + 瘦身 + 死 store 消除 + overlay）。
    const fix = b.addSystemCommand(&.{ mcstools, "fix", "uart_echo.asm" });
    fix.step.dependOn(&z.step);
    const optm = b.addSystemCommand(&.{ mcstools, "opt", "uart_echo.asm" });
    optm.step.dependOn(&fix.step);
    const ir = b.addSystemCommand(&.{ mcstools, "ir", "uart_echo.asm" });
    ir.step.dependOn(&optm.step);
    // overlay 对 mcs251（@spx 帧）是空操作，但主工程 postprocess_asm 会跑它（顺带把行尾写成 CRLF），
    // 加上以保持中间 .asm 与主工程逐字节一致。
    const ovl = b.addSystemCommand(&.{ mcstools, "overlay", "uart_echo.asm" });
    ovl.step.dependOn(&ir.step);

    // [5/5] asm -> rel -> ihx
    const as = b.addSystemCommand(&.{ sdas, "-plosgffw", "uart_echo.rel", "uart_echo.asm" });
    ...
    link.addArgs(&.{ "crt0.rel", "uart_echo.rel", "-o", "uart_echo.ihx" });
