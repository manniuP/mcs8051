...
rem [1/5] Zig -> asm
"%ZIG%" build-obj !OPT! -target !TARGET! --dep mcs -Mroot=uart_echo.zig -Mmcs=mcs251.zig -femit-bin=uart_echo.asm || goto :fail
rem [2/5] fix label collisions
"%MCSTOOLS%" fix uart_echo.asm || goto :fail
rem [3/5] shrink
"%MCSTOOLS%" opt uart_echo.asm || goto :fail
rem [4/5] dead-store elimination
"%MCSTOOLS%" ir uart_echo.asm || goto :fail
rem [4b/5] overlay pass (no-op for mcs251; normalizes line endings for byte-identical output)
"%MCSTOOLS%" overlay uart_echo.asm || goto :fail
rem [5/5] asm -> rel -> ihx
"!SDAS!" -plosgffw uart_echo.rel uart_echo.asm || goto :fail
"!SDAS!" -plosgffw crt0.rel !CRT0! || goto :fail
"%SDCC%" !MODEL! --model-large --code-loc !CODE_LOC! !LINKOPT! crt0.rel uart_echo.rel -o uart_echo.ihx || goto :fail
popd
echo [OK] uart_echo.ihx
exit /b 0
