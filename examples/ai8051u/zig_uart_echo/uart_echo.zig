...
const SBUF_ADDR: u8 = @intCast(dev.sfr.SBUF);
...
fn uartInitT2(comptime fosc: u32, comptime baud: u32) void {
    m.sfrWrite(SCON_ADDR, 0x50); // SCON：模式1，REN=1（允许接收）
    ...
    m.sfrAnd(P_SW1_ADDR, 0x3f); // P_SW1：UART1 选 P3.0(RxD)/P3.1(TxD)
    m.sfrAnd(P3M1_ADDR, ~@as(u8, 0x03)); // P3M1.0/P3M1.1 = 0
    m.sfrOr(P3M0_ADDR, 0x02); // P3M0.1=1：P3.1 推挽（TxD）；P3.0 保持准双向（RxD）
    m.bitSet(IE_ADDR, 4); // IE.ES = 1：允许 UART1 中断
    m.bitSet(IE_ADDR, 7); // IE.EA = 1：总中断
}
...
export fn on_uart() void {
    ...
    if ((m.sfrPtr(SCON_ADDR).* & 0x02) != 0) {
        m.sfrAnd(SCON_ADDR, ~@as(u8, 0x02)); // 清 TI
        if (r_head != r_tail) {
            m.sfrPtr(SBUF_ADDR).* = ring[r_tail];
            r_tail +%= 1;
        } else {
            tx_busy = 0;
        }
    ...
    if ((m.sfrPtr(SCON_ADDR).* & 0x01) != 0) {
        const b = m.sfrPtr(SBUF_ADDR).*; // 读 SBUF
        m.sfrAnd(SCON_ADDR, ~@as(u8, 0x01)); // 显式清 RI（读 SBUF 本已清，双保险；某些仿真模型不自动清）
        if (tx_busy == 0) {
            m.sfrPtr(SBUF_ADDR).* = b; // 回写 SBUF → 回显
            tx_busy = 1;
        } else {
            const h = r_head +% 1;
            if (h != r_tail) { // 未满
