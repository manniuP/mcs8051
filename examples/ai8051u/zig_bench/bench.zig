...
fn t0Init() void {
    m.sfrAnd(TMOD_ADDR, 0xF0); // TMOD: T0 模式0（16 位自动重装）、C/T=0、GATE=0
    m.sfrOr(AUXR_ADDR, 0x80); // AUXR.T0x12 = 1  → 1T（每时钟 +1）
    m.sfrAnd(TCON_ADDR, 0xEF); // TCON.TR0 = 0（停止）
    m.sfrWrite(TH0_ADDR, 0x00); // TH0 = 0（同时写 RL_TH0）
    m.sfrWrite(TL0_ADDR, 0x00); // TL0 = 0（同时写 RL_TL0）
    m.sfrOr(TCON_ADDR, 0x10); // TCON.TR0 = 1（自由计数）
}

fn t0Now() u16 {
    // 稳健读：TL0, TH0, TL0；若 TL0 在读数期间变化则重读 TH0（处理溢出边界）。
    const l0: u16 = m.sfrPtr(TL0_ADDR).*;
    var h: u16 = m.sfrPtr(TH0_ADDR).*;
    const l1: u16 = m.sfrPtr(TL0_ADDR).*;
    if (l1 != l0) h = m.sfrPtr(TH0_ADDR).*;
    return (h << 8) | l1;
}
