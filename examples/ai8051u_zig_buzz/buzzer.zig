...
inline fn playNote(hi: u8, lo: u8, t_hi: u8, t_lo: u8) void {
    ...
    p_lo.* = 0;
    p_hi.* = 0;
    m.sfrPtr(0x8c).* = hi; // TH0
    m.sfrPtr(0x8a).* = lo; // TL0
    ...
    m.bitClr(0x88, 4); // TR0 = 0
    m.bitSet(0xb0, 6); // P3.6 = 1（停声）
    busy(2000); // 音符间小间隙
}

export fn main() void {
    m.sfrAnd(0xb1, ~@as(u8, 0x40)); // P3M1.6 = 0
    m.sfrOr(0xb2, 0x40); // P3M0.6 = 1（推挽）
    m.bitSet(0xb0, 6); // P3.6 = 1

    m.sfrWrite(0x89, 0x00); // TMOD：Timer0 模式0（16 位自动重载）
    m.sfrAnd(0x8e, 0x7f); // AUXR.7=0 → Timer0 12T
    m.bitClr(0x88, 4); // TR0 = 0
