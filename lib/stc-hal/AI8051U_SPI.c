...
__bit B_SPI_Busy; //发送忙标志
__bit SPI_RxTimerOut;
__bit SPI_TimerOutEn;
u8 	SPI_RxCnt;
...
{
    if(SPIx->SPI_SSIG == ENABLE) SPCTL |= 0x80;     //conform Master or Slave by SPI_Mode(ignore SS)
    else                         SPCTL &= ~0x80;     //conform Master or Slave by SS pin.
