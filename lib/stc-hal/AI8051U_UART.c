...
u8 UART_Configuration(u8 UARTx, COMx_InitDefine *COMx)
{
    ...
    if(UARTx == UART1)
    {
        ...
        if((COMx->UART_Mode == UART_9bit_BRTx) || (COMx->UART_Mode == UART_8bit_BRTx))  //可变波特率
        {
            ...
            {
                AUXR &= ~0x10;        //Timer stop
                AUXR |= 0x01;      //S1 BRT Use Timer2;
                AUXR &= ~0x08;      //Timer2 set As Timer
                AUXR |= 0x04;      //Timer2 set as 1T mode
                T2H = (u8)(j>>8);
                T2L = (u8)j;
                AUXR |= 0x10;        //Timer run enable
            }
            else
            {
                TR1 = 0;
                AUXR &= ~0x01;      //S1 BRT Use Timer1;
                TMOD &= ~0x40;      //Timer1 set As Timer
        ...
        {
            if(COMx->BaudRateDouble == ENABLE)  AUXR |= 0x20;    //固定波特率SysClk/2
            else                                AUXR &= ~0x20;    //固定波特率SysClk/12
        }
        else if(COMx->UART_Mode == UART_9bit)    //固定波特率SysClk*2^SMOD/64
        {
            if(COMx->BaudRateDouble == ENABLE)  PCON |= 0x80;    //固定波特率SysClk/32
            else                                PCON &= ~0x80;    //固定波特率SysClk/64
        }
    ...
    if(UARTx == UART3)
    {
        ...
        if((COMx->UART_Mode == UART_9bit_BRTx) || (COMx->UART_Mode == UART_8bit_BRTx))  //可变波特率
        {
            ...
            {
                T4T3M &= ~0x08;    //Timer stop
                S3_BRT_UseTimer3(); //S3 BRT Use Timer3;
                T3H = (u8)(j>>8);
                T3L = (u8)j;
                T4T3M &= ~0x04;  //Timer3 set As Timer
                T4T3M |= 0x02;  //Timer3 set as 1T mode
                T4T3M |= 0x08;    //Timer run enable
            }
        }
        else return FAIL;    //模式错误
    ...
    if(UARTx == UART4)
    {
        ...
        if((COMx->UART_Mode == UART_9bit_BRTx) || (COMx->UART_Mode == UART_8bit_BRTx))    //可变波特率
        {
            ...
            {
                T4T3M &= ~0x80;        //Timer stop
                S4_BRT_UseTimer4(); //S4 BRT Use Timer4;
                T4H = (u8)(j>>8);
                T4L = (u8)j;
                T4T3M &= ~0x40;      //Timer4 set As Timer
                T4T3M |= 0x20;      //Timer4 set as 1T mode
                T4T3M |= 0x80;        //Timer run enable
            }
        }
        else return FAIL;    //模式错误
