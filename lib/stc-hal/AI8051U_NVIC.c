...
//========================================================================
// 函数: NVIC_Timer11_Init
// 描述: Timer11嵌套向量中断控制器初始化.
// 参数: State:    中断使能状态, ENABLE/DISABLE.
// 参数: Priority: 中断优先级, NONE.
// 返回: 执行结果 SUCCESS/FAIL.
// 版本: V1.0, 2025-02-10
...
{
	if(State <= ENABLE) Timer11_Interrupt(State); else  return FAIL;
	Priority = NONE;
	return SUCCESS;
}
...
{
	if(State & RISING_EDGE)	CMPCR1 |= 0x20;			//允许上升沿中断
	else	CMPCR1 &= ~0x20;			//禁止上升沿中断
	if(State & FALLING_EDGE)	CMPCR1 |= 0x10;		//允许下降沿中断
	else	CMPCR1 &= ~0x10;			//禁止上升沿中断
...
// 函数: NVIC_QSPI_Init
// 描述: QSPI嵌套向量中断控制器初始化.
// 参数: State:    中断使能状态, QSPI_SMIE/QSPI_FTIE/QSPI_TCIE/QSPI_TEIE/DISABLE.
// 参数: Priority: 中断优先级, Priority_0,Priority_1,Priority_2,Priority_3.
// 返回: 执行结果 SUCCESS/FAIL.
// 版本: V1.0, 2025-02-08
//========================================================================
...
{
	if(State <= ENABLE) QSPI_Interrupt(State); else  return FAIL;
	if(Priority <= Priority_3) QSPI_Priority(Priority); else  return FAIL;
	return SUCCESS;
}
